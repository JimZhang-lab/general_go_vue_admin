 ...
 * @FilePath: /web/src/views/Auth/RoleManagement.vue
 * @Description: 
 * 
 */
<template>
...
const getRoleList = async () => {
  try {
    ...
    const { data: res } = await adminApi.getRoleList(params)
    if (res.code === 200) {
      roleList.value = res.data.list || []
      pagination.total = res.data.total || 0
      pagination.pages = Math.ceil((res.data.total || 0) / pagination.pageSize)
    } else {
      ToastAlert.error({ title: '获取角色列表失败', message: res.message })
    }
  } catch (error) {
    console.error('获取角色列表失败:', error)
    ToastAlert.error({ title: '获取角色列表失败', message: '网络异常，请重试' })
  } finally {
    loading.value = false
  }
}
...
const getMenuTree = async () => {
  try {
    ...
    if (res.code === 200) {
      ...
      menuTree.value = buildMenuTree(list as any)
    }
  } catch (error) {
...
const loadRolePermissions = async (roleId: number) => {
  try {
    const { data: res } = await adminApi.getRolePermissionIds(roleId)
    if (res.code === 200) {
      selectedMenuIds.value = (Array.isArray(res.data) ? res.data : []) as number[]
    }
  } catch (e) {
...
const savePermissions = async () => {
  ...
  try {
    const { data: res } = await adminApi.assignRolePermissions({ id: currentRole.value.id, menuIds: selectedMenuIds.value })
    if (res.code === 200) {
      ToastAlert.success({ title: '保存成功', message: '权限已保存' })
      showPermissionModal.value = false
    } else {
      ToastAlert.error({ title: '保存失败', message: res.message })
    }
  } catch (error) {
    console.error('保存权限失败:', error)
    ToastAlert.error({ title: '保存失败', message: '网络异常，请重试' })
  }
}
