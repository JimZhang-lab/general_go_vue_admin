/*
 * @Author: JimZhang
 * @Date: 2023-07-23 15:33:20
 * @LastEditors: 很拉风的James
 * @LastEditTime: 2025-07-25 20:30:43
 * @FilePath: /server/api/controller/sysLoginInfo.go
 * @Description:
 *
 */
...
// @Summary 导出登录日志接口
// @Produce json
// @Description 导出登录日志接口
// @Param username query string false "用户名"
// @Param loginStatus query string false "登录状态（1-成功 2-失败）"
// @Param beginTime query string false "开始时间"
// @Param endTime query string false "结束时间"
// @Success 200 {object} result.Result
// @router /api/sysLoginInfo/export [get]
// @Security ApiKeyAuth
func ExportSysLoginInfo(c *gin.Context) {
	Username := c.Query("username")
	LoginStatus := c.Query("loginStatus")
	BeginTime := c.Query("beginTime")
	EndTime := c.Query("endTime")
	service.SysLoginInfoService().ExportSysLoginInfoList(c, Username, LoginStatus, BeginTime, EndTime)
}
