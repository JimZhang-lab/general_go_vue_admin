/*
 * @Author: JimZhang
 * @Date: 2025-07-25 00:43:08
 * @LastEditors: 很拉风的James
 * @LastEditTime: 2025-07-25 00:57:16
 * @FilePath: /server/api/dao/sysAdmin.go
 * @Description:
 *
 */
package dao
...
// 分页查询用户列表
func GetSysAdminList(PageSize, PageNum int, Username, Phone, Status, BeginTime, EndTime string) (sysAdminVo []entity.SysAdminVo, count int64) {
	curDb := db.Db.Table("sys_admin").
		Select("sys_admin.*, sys_admin_role.role_id, sys_post.post_name, sys_role.role_name, sys_dept.dept_name").
	...
	if Phone != "" {
		curDb = curDb.Where("sys_admin.phone LIKE ?", "%"+Phone+"%")
	}
	if Status != "" {
